    ...
    <table class="table table-bordered table-hover">
      <thead>
        <tr>
          ...
          <th scope="col">Купленные квесты</th> {{-- Новая колонка --}}
      ...
      <tbody>
        @foreach($users as $key => $user)
          <tr>
            ...
            <td>
              @if($user->purchasedQuests && $user->purchasedQuests->count() > 0)
                <div class="d-flex flex-wrap gap-1">
                  @foreach($user->purchasedQuests as $quest)
                    <a href="{{ route('admin.quests.edit', $quest->id) }}" class="quest-badge text-decoration-none"
            ...
            <td>
              <div class="d-flex">
                ...
                <form method="post" action="{{ route('admin.users.destroy', $user->id) }}">
                  ...
                  <button onclick="return confirm('Вы уверены, что хотите удалить этот элемент?')" class="delete-button"
                    type="submit">
